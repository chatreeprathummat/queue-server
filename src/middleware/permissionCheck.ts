...
const { 
    ...
    createTokenHeaders, 
    handleJWTError
} = require("../utils/authUtils") as any;
const { 
    logPermissionCheck
} = require("../services/logging/authLogger") as any;
const { wrapController } = require("../services/logging/controllers/requestLogger") as any;
const permissionMoment = require('moment') as any;
 ...
 */
export const authCheck = wrapController(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const authStartTime: number = Date.now();
    ...
    try {
        ...
        try {
            ...
            const decoded: DecodedToken = jwt.verify(token, process.env.SECRET_KEY);
            ...
            const result: AuthResult = await db.executeTransaction(async (connection: any) => {
                ...
                const [rows]: [TokenRow[], any] = await connection.execute(checkTokenSql, [token]);
                ...
                if (rows.length === 0) {
                    ...
                    const checkExpiredSql: string = `
                        SELECT t.*, u.username
                        FROM tbl_ur_auth_tokens t
                        JOIN tbl_ur_auth_users u ON t.username = u.username
                        WHERE t.token = ?
                    `;
                    const [expiredRows]: [TokenRow[], any] = await connection.execute(checkExpiredSql, [token]);
                ...
                const currentTime: string = permissionMoment().format('YYYY-MM-DD HH:mm:ss');
                const updateLastActiveSql: string = `
            ...
            const totalDuration: number = Date.now() - authStartTime;
...
export const checkPermission = (permissionCode: string) => {
    return wrapController(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        const permissionStartTime: number = Date.now();
        ...
        try {
            ...
            const db = ManagementDB.getInstance();
            const permissionCheckSql: string = `
                SELECT 
                    p.permission_code,
                    p.permission_name,
                    r.role_name
            ...
            const permissionRows: any[] = await db.executeQuery(
                permissionCheckSql, 
                [username, permissionCode],
                'checkPermission'
            );

            const totalDuration: number = Date.now() - permissionStartTime;
 ...
 */
export const checkAnyPermission = (permissionCodes: string[]) => {
    return wrapController(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        const permissionStartTime: number = Date.now();
        ...
        try {
            ...
            const db = ManagementDB.getInstance();
            const placeholders: string = permissionCodes.map(() => '?').join(',');
            const permissionCheckSql: string = `
                SELECT 
                    p.permission_code,
                    p.permission_name,
            ...
            const foundPermissions: string[] = permissionRows.map((row: any) => row.permission_code);
