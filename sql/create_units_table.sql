...
BEGIN
    ...
    DECLARE v_existing_unit_code VARCHAR(20) DEFAULT NULL;
    ...
    -- ตรวจสอบว่ามีรหัสหน่วยนับนี้อยู่แล้วหรือไม่ (ถ้ามีการส่งรหัสมา)
    -- หมายเหตุ: ไม่ตรวจสอบ unit_code ซ้ำกันแล้ว เพราะอาจมีหลายหน่วยนับใช้รหัสเดียวกันได้
    -- IF p_unit_code IS NOT NULL AND p_unit_code != '' THEN
    --     SELECT unit_id, unit_code INTO v_existing_unit_id, v_existing_unit_code
    --     FROM tbl_inv_units 
    --     WHERE LOWER(TRIM(unit_code)) = LOWER(TRIM(p_unit_code))
    --     AND is_active = 1;
    -- END IF;
    ...
    IF v_existing_unit_id IS NOT NULL THEN
        SET p_result_code = 0;
        SET p_result_message = CONCAT('หน่วยนับ "', COALESCE(v_existing_unit_name, v_existing_unit_code), '" มีอยู่ในระบบแล้ว');
...
BEGIN
    ...
    -- ตรวจสอบรหัสหน่วยนับ (ไม่ตรวจสอบซ้ำกันแล้ว เพราะอาจมีหลายหน่วยนับใช้รหัสเดียวกันได้)
    -- IF NEW.unit_code != OLD.unit_code AND NEW.unit_code IS NOT NULL AND NEW.unit_code != '' THEN
    --     IF EXISTS (
    --         SELECT 1 FROM tbl_inv_units 
    --         WHERE LOWER(TRIM(unit_code)) = LOWER(TRIM(NEW.unit_code))
    --         AND unit_id != NEW.unit_id
    --         AND is_active = 1
    --     ) THEN
    --         SIGNAL SQLSTATE '45000'
    --         SET MESSAGE_TEXT = 'รหัสหน่วยนับนี้มีอยู่ในระบบแล้ว';
    --     END IF;
    -- END IF;
END //
